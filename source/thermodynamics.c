...
int thermodynamics_recombination_with_hyrec(
  ...
  class_call(background_w_fld(pba,pba->a_today,&w_fld,&dw_over_da_fld,&integral_fld,0), pba->error_message, pth->error_message);
  param.w0 = w_fld;
...
int thermodynamics_recombination_with_recfast(
  ...
  // if(zinitial <  ppr->recfast_z_He_3){
  //   y[0] = 3e-4;
  //   y[1] = 0;
  //   x0 = 3e-4;
  //   y[2] = preco->Tnow*(1.+z);
...
int thermodynamics_derivs_with_recfast(
  ...
  if (timeTh < preco->H_frac*timeH) {
    ...
    //   Tmat = 1;
    // }
    // printf("dy[2] %e Tmat %e x %e z %e Hz %e\n",dy[2],Tmat,x,z,Hz);
  }
